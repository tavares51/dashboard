    ...
    try:
        QUERY = """
            ...
            FROM NOTA_FISCAL AS NFI
                OUTER APPLY (
                SELECT TOP (1) CTR.CTR_VALOR_TOTAL
                FROM CONHECIMENTO_TRANSPORTE AS CTR
                WHERE CTR.CTR_DEST_CPFCNPJ = NFI.NFI_CNPJ
                    AND CTR.CTR_DATA_EMISSAO = NFI.NFI_DATA_EMISSAO
                ORDER BY CTR.CTR_ID DESC  -- ou outro critério estável
                ) AS C
